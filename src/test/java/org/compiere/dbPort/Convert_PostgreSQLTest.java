...
public final class Convert_PostgreSQLTest extends TestCase {
  ...
  public void test1751966() {
    ...
    sqe =
        "UPDATE I_ReportLine SET Description=ii.Description,SeqNo=ii.SeqNo,IsSummary=ii.IsSummary,"
  ...
  public void test1707959() {
    sql =
        "UPDATE AD_PrintFormatItem_Trl new "
  ...
  public void testAlterColumn() {
    ...
    // sqe = "ALTER TABLE Test ALTER COLUMN T_Integer TYPE NUMERIC(10); ALTER TABLE Test ALTER
    // COLUMN T_Integer SET NOT NULL;";
    ...
    // sqe = "ALTER TABLE Test ALTER COLUMN T_Integer TYPE NUMERIC(10); ALTER TABLE Test ALTER
    // COLUMN T_Integer DROP NOT NULL;";
    ...
    // sqe = "ALTER TABLE LPI_Publication ALTER COLUMN AD_Client_ID TYPE NUMERIC(10); ALTER TABLE
    // LPI_Publication ALTER COLUMN AD_Client_ID SET DEFAULT NULL; ";
    sqe =
        "INSERT INTO t_alter_column values('lpi_publication','AD_Client_ID','NUMERIC(10)',null,'NULL')";
    r = convert.convert(sql);
    assertEquals(sqe, r[0]);
    ...
    sqe =
        "INSERT INTO t_alter_column values('sometablename','someColumnName','VARCHAR(64)',null,null)";
    r = convert.convert(sql);
    assertEquals(sqe, r[0].trim());
    ...
    sqe =
        "INSERT INTO t_alter_column values('pp_order_nodenext','PP_Order_NodeNext_ID',null,'NULL',null)";
    r = convert.convert(sql);
    assertEquals(sqe, r[0].trim());

    sql = "ALTER TABLE C_InvoiceTax ADD Created DATE DEFAULT SYSDATE NOT NULL";
    sqe =
        "ALTER TABLE C_InvoiceTax ADD COLUMN Created TIMESTAMP DEFAULT statement_timestamp() NOT NULL";
    r = convert.convert(sql);
    assertEquals(sqe, r[0].trim());
  }
  ...
  public void test1704261() {
    ...
    sql =
        "UPDATE I_Conversion_Rate i SET MultiplyRate = 1 / DivideRate WHERE (MultiplyRate IS NULL OR MultiplyRate = 0) AND DivideRate IS NOT NULL AND DivideRate<>0 AND I_IsImported<>'Y' AND AD_Client_ID=1000000";
    sqe =
        "UPDATE I_Conversion_Rate SET MultiplyRate = 1 / DivideRate WHERE (MultiplyRate IS NULL OR MultiplyRate = 0) AND DivideRate IS NOT NULL AND DivideRate<>0 AND I_IsImported<>'Y' AND AD_Client_ID=1000000";
    r = convert.convert(sql);
    assertEquals(sqe, r[0]);
  }
  ...
  public void testMultiColumnAssignment() {
    ...
    sqe =
        "UPDATE T_Report SET Name=e.Name,Description=fa.Description FROM Fact_Acct fa INNER JOIN AD_Table t ON (fa.AD_Table_ID=t.AD_Table_ID) INNER JOIN AD_Element e ON (t.TableName||'_ID'=e.ColumnName) WHERE T_Report.Fact_Acct_ID=fa.Fact_Acct_ID AND T_Report.Fact_Acct_ID <> 0 AND T_Report.AD_PInstance_ID=0";
    r = convert.convert(sql);
    assertEquals(sqe, r[0]);
  ...
  public void testReservedWordInQuote() {
    ...
    sql =
        "UPDATE AD_Message_Trl SET MsgText='{0} Linea(s) {1,number,#,##0.00}  - Total: {2,number,#,##0.00}',MsgTip=NULL,Updated=TO_DATE('2007-01-12 21:44:31','YYYY-MM-DD HH24:MI:SS'),IsTranslated='Y' WHERE AD_Message_ID=828 AND AD_Language='es_MX'";
    sqe =
        "UPDATE AD_Message_Trl SET MsgText='{0} Linea(s) {1,number,#,##0.00}  - Total: {2,number,#,##0.00}',MsgTip=NULL,Updated=TO_TIMESTAMP('2007-01-12 21:44:31','YYYY-MM-DD HH24:MI:SS'),IsTranslated='Y' WHERE AD_Message_ID=828 AND AD_Language='es_MX'";
    r = convert.convert(sql);
    assertEquals(sqe, r[0]);
  }
  ...
  public void test1580231() {
    ...
    sql =
        "UPDATE t_report"
            ...
            + " WHERE record_id <> 0 "
            + " AND ad_pinstance_id = 1000024 "
            + " AND pa_reportline_id = 101 "
            + " AND fact_acct_id = 0 ";
    sqe =
        "UPDATE t_report SET NAME=c_elementvalue.VALUE,description=c_elementvalue.NAME FROM c_elementvalue WHERE c_elementvalue.c_elementvalue_id = t_report.record_id AND t_report.record_id <> 0 AND t_report.ad_pinstance_id = 1000024 AND t_report.pa_reportline_id = 101 AND t_report.fact_acct_id = 0";
    r = convert.convert(sql);
    assertEquals(sqe, r[0]);
  }
  ...
  public void testAliasInUpdate() {
    ...
    sqe =
        "UPDATE I_Order SET C_BPartner_ID=u.C_BPartner_ID,AD_User_ID=u.AD_User_ID FROM AD_User u WHERE I_Order.ContactName=u.Name AND I_Order.AD_Client_ID=u.AD_Client_ID AND u.C_BPartner_ID IS NOT NULL AND I_Order.C_BPartner_ID IS NULL AND I_Order.ContactName IS NOT NULL AND EXISTS (SELECT Name FROM AD_User u WHERE I_Order.ContactName=u.Name AND I_Order.AD_Client_ID=u.AD_Client_ID AND u.C_BPartner_ID IS NOT NULL GROUP BY Name HAVING COUNT(*)=1) AND I_Order.I_IsImported<>'Y' AND I_Order.AD_Client_ID=11";
    r = convert.convert(sql);
    assertEquals(sqe, r[0]);
  }
  ...
  public void testSubQuery() {
    ...
    sql =
        "INSERT INTO "
            ...
            + "')";
    sqe =
        "INSERT INTO AD_Column_Trl(AD_Language,IsTranslated, AD_Client_ID,AD_Org_ID, Createdby,UpdatedBy, AD_Column_ID,Name) SELECT 'es_MX','N', AD_Client_ID,AD_Org_ID, 100,100, AD_Column_ID,Name FROM AD_Column WHERE AD_Column_ID NOT IN (SELECT AD_Column_ID FROM AD_Column_Trl WHERE AD_Language='es_MX')";
    r = convert.convert(sql);
    assertEquals(sqe, r[0]);
  }
  ...
  public void test1622302() {
    ...
    sql =
        "DELETE FROM M_InOutLineMA ma WHERE EXISTS "
            + "(SELECT * FROM M_InOutLine l WHERE l.M_InOutLine_ID=ma.M_InOutLine_ID"
            + " AND M_InOut_ID=0)";
    sqe =
        "DELETE FROM M_InOutLineMA WHERE EXISTS (SELECT * FROM M_InOutLine l WHERE l.M_InOutLine_ID=M_InOutLineMA.M_InOutLine_ID AND M_InOut_ID=0)";
    r = convert.convert(sql);
    assertEquals(sqe, r[0]);
  }
  ...
  public void test1638046() {
    ...
    r = convert.convert(sql);
    sqe =
        "UPDATE GL_JournalBatch SET TotalDr="
  ...
  public void testDecode() {
    sql =
        "SELECT supplier_name, decode(supplier_id, 10000, 'IBM', 10001, 'Microsoft', 10002, 'Hewlett Packard', 'Gateway') FROM suppliers";
    sqe =
        "SELECT supplier_name, CASE WHEN supplier_id=10000 THEN 'IBM' WHEN supplier_id=10001 THEN 'Microsoft' WHEN supplier_id=10002 THEN 'Hewlett Packard' ELSE 'Gateway' END FROM suppliers";
    r = convert.convert(sql);
    assertEquals(sqe, r[0]);
  ...
  public void testCasts() {
    ...
    String[][] sql_tests =
        new String[][] {
          ...
          {
            "cast('this is a string as a ''string''' as nvarchar2(40))",
            "cast('this is a string as a ''string''' as VARCHAR)"
          },
          {"cast(tbl.IsView as char)", "cast(tbl.IsView as char)"},
          {
            "cast(trunc(tbl.Updated,'MONTH') as date)",
            "cast(trunc(tbl.Updated,'MONTH') as TIMESTAMP)"
          },
  ...
  public void test3137355() {
    sql =
        "INSERT INTO MyTable (a, b, c, d, xml) VALUES ('val1', 'val2', 'this ''is'' a string with ''quotes'' and backslashes ''\\''', 'val4')";
    sqe =
        "INSERT INTO MyTable (a, b, c, d, xml) VALUES ('val1', 'val2', E'this ''is'' a string with ''quotes'' and backslashes ''\\\\''', 'val4')";
    r = convert.convert(sql);
    assertEquals(sqe, r[0]);

    sql =
        "SELECT AD_Tab.AD_Tab_ID,NULL,COALESCE(AD_Tab.Name,'-1') ||'_'|| COALESCE((SELECT COALESCE(AD_Window.Name,'') FROM AD_Window WHERE AD_Tab.AD_Window_ID=AD_Window.AD_Window_ID),'-1'),AD_Tab.IsActive"
