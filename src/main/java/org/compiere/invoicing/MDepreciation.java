...
public class MDepreciation extends X_A_Depreciation
{
	...
	s_cache = new CCache<Integer,MDepreciation>(I_A_Depreciation.Table_Name, 5);
	/**		Cache for type							*/
	private static CCache<String,MDepreciation>
	s_cache_forType = new CCache<String,MDepreciation>(I_A_Depreciation.Table_Name, I_A_Depreciation.Table_Name+"_DepreciationType", 5);
	/**		Static logger							*/
	...
	public static MDepreciation get(Properties ctx, String depreciationType)
	{
		...
		final String whereClause = I_A_Depreciation.COLUMNNAME_DepreciationType+"=?"
									+" AND AD_Client_ID IN (0,?)";
		depr = new Query(ctx, I_A_Depreciation.Table_Name, whereClause, null)
								...
								int A_Current_Period, BigDecimal Accum_Dep)
	{
		...
		{
			throw new AssetNotSupportedException(I_A_Depreciation.COLUMNNAME_DepreciationType, depreciationType);
		}
		//
		if (retValue == null)
		{
