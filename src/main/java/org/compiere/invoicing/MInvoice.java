...
public class MInvoice extends X_C_Invoice implements DocAction, I_C_Invoice, IPODoc
{
	...
	{
		List<MInvoice> list = new Query(ctx, I_C_Invoice.Table_Name, I_C_Invoice.COLUMNNAME_C_BPartner_ID+"=?", trxName)
	...
	/**	Cache						*/
	private static CCache<Integer,MInvoice>	s_cache	= new CCache<Integer,MInvoice>(I_C_Invoice.Table_Name, 20, 2);	//	2 minutes
	...
	public int copyLinesFrom (MInvoice otherInvoice, boolean counter, boolean setOrder)
	{
		...
		for (int i = 0; i < fromLines.length; i++)
		{
			...
			if (counter)
			{
				...
				{
					org.compiere.order.MInOutLine peer = new org.compiere.order.MInOutLine(getCtx(), fromLine.getM_InOutLine_ID(), get_TrxName());
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if (!newRecord && (is_ValueChanged(I_C_Invoice.COLUMNNAME_M_PriceList_ID) || is_ValueChanged(I_C_Invoice.COLUMNNAME_DateInvoiced))) {
			...
			if (cnt > 0) {
				...
				if (is_ValueChanged(I_C_Invoice.COLUMNNAME_DateInvoiced)) {
					...
					MPriceListVersion plOld = pList.getPriceListVersion((Timestamp)get_ValueOld(I_C_Invoice.COLUMNNAME_DateInvoiced));
					MPriceListVersion plNew = pList.getPriceListVersion((Timestamp)get_Value(I_C_Invoice.COLUMNNAME_DateInvoiced));
		...
		if (! recursiveCall && (!newRecord && is_ValueChanged(I_C_Invoice.COLUMNNAME_C_PaymentTerm_ID))) {
			recursiveCall = true;
			try {
		...
		if (getC_Order_ID() != 0) {
			...
			for (int i = 0; i < lines.length; i++) {
				if (lines[i].getM_InOutLine_ID() > 0) {
					org.compiere.order.MInOutLine sLine = new org.compiere.order.MInOutLine(getCtx(), lines[i].getM_InOutLine_ID(), get_TrxName());
...
//		}
		...
		if (   getGrandTotal().signum() != 0
			&& (X_C_Invoice.PAYMENTRULE_OnCredit.equals(getPaymentRule()) || X_C_Invoice.PAYMENTRULE_DirectDebit.equals(getPaymentRule())))
		{
	...
	public String completeIt()
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			if (!isSOTrx()
				...
				&& !isReversal())
			{
				org.compiere.order.MInOutLine receiptLine = new org.compiere.order.MInOutLine(getCtx(),line.getM_InOutLine_ID(), get_TrxName());
...
//		counter.setBPartner(counterBP);// was set on copyFrom
			...
			|| X_C_Invoice.DOCSTATUS_NotApproved.equals(getDocStatus()) )
		{
			...
			for (int i = 0; i < lines.length; i++)
			{
				...
				if (old.compareTo(Env.ZERO) != 0)
				{
					...
					{
						org.compiere.order.MInOutLine ioLine = new org.compiere.order.MInOutLine(getCtx(), line.getM_InOutLine_ID(), get_TrxName());
	...
	private MInvoice reverse(boolean accrual) {
		...
		for (int i = 0; i < iLines.length; i++)
		{
			...
			{
				org.compiere.order.MInOutLine ioLine = new MInOutLine(getCtx(), iLine.getM_InOutLine_ID(), get_TrxName());
				ioLine.setIsInvoiced(false);
				ioLine.saveEx(get_TrxName());
				//	Reconsiliation
