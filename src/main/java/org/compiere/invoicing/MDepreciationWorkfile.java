	...
	implements UseLife
{
	...
	*/ // end comment by @win
	...
	{
		return new Query(ctx, I_A_Depreciation_Workfile.Table_Name, I_A_Depreciation_Workfile.COLUMNNAME_A_Asset_ID +"=?", trxName)
	...
	public static MDepreciationWorkfile get (Properties ctx, int A_Asset_ID, String postingType, String trxName)
	{
		...
		MDepreciationWorkfile wk = new Query(ctx, MDepreciationWorkfile.Table_Name, whereClause, trxName)
											...
											.firstOnly();
		*/
		final String whereClause = I_A_Depreciation_Workfile.COLUMNNAME_A_Asset_ID+"=?"
									+" AND "+ I_A_Depreciation_Workfile.COLUMNNAME_PostingType+"=? ";
		MDepreciationWorkfile wk = new Query(ctx, I_A_Depreciation_Workfile.Table_Name, whereClause, trxName)
	...
	s_cacheAsset = new CCache<MultiKey, MDepreciationWorkfile>(I_A_Depreciation_Workfile.Table_Name, I_A_Depreciation_Workfile.Table_Name+"_Asset", 10);
...
//		}
	...
	{
		//Own contribution:
		BigDecimal valCofinantare = SetGetUtil.get_AttrValueAsBigDecimal(m, I_A_Depreciation_Workfile.COLUMNNAME_A_Valoare_Cofinantare);
		// Asset Value:
		BigDecimal assetCost = SetGetUtil.get_AttrValueAsBigDecimal(m, I_A_Depreciation_Workfile.COLUMNNAME_A_Asset_Cost);
		// Third value:
		BigDecimal valTert = SetGetUtil.get_AttrValueAsBigDecimal(m, I_A_Depreciation_Workfile.COLUMNNAME_A_Valoare_Tert);
		...
		else if (I_A_Depreciation_Workfile.COLUMNNAME_A_Valoare_Cofinantare.equals(changedColumnName))
		{
		...
		if (X_A_Depreciation_Workfile.A_TIP_FINANTARE_Proprie.equals(tipFinantare) && SetGetUtil.isPersistent(m))
		{
