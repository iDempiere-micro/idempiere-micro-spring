...
public class MInvoiceLine extends X_C_InvoiceLine implements I_C_InvoiceLine, IDocLine
{
	...
	public static MInvoiceLine getOfInOutLineFromMatchInv(org.compiere.order.MInOutLine sLine) {
		...
		List<MInvoiceLine> list = new Query(sLine.getCtx(), Table_Name,whereClause,sLine.get_TrxName())
		.setParameters(sLine.getM_InOutLine_ID())
		.list();
	...
	protected boolean afterDelete (boolean success)
	{
		...
		{
			org.compiere.order.MInOutLine sLine = new org.compiere.order.MInOutLine(getCtx(), getM_InOutLine_ID(), get_TrxName());
	...
	public String allocateLandedCosts()
	{
		...
		for (int ii = 0; ii < lcs.length; ii++)
		{
			...
			{
				org.compiere.order.MInOutLine iol = new org.compiere.order.MInOutLine(getCtx(), lc.getM_InOutLine_ID(), get_TrxName());
				if (!iol.isDescription() && iol.getM_Product_ID() != 0)
					list.add(iol);
			}
		}
    ...
    public void setClientOrg(IPO po) { super.setClientOrg(po);}
