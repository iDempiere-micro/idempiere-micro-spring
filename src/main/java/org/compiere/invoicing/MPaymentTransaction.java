...
public class MPaymentTransaction extends X_C_PaymentTransaction implements ProcessCall, PaymentInterface {
	...
	public boolean processOnline()
	{
		...
		if(getTrxType().equals(X_C_PaymentTransaction.TRXTYPE_Void) || getTrxType().equals(X_C_PaymentTransaction.TRXTYPE_CreditPayment))
		{
			if (isVoided())
			{
...
//				}
	...
	public static IPaymentProcessor createPaymentProcessor(MBankAccountProcessor mbap, PaymentInterface mp) {
	...
	{
		if (getTenderType().equals(X_C_PaymentTransaction.TENDERTYPE_CreditCard) && isOnline() && getTrxType().equals(X_C_PaymentTransaction.TRXTYPE_Authorization) && !isVoided() && !isDelayedCapture())
		{
			boolean ok = false;
			try
			{
				MPaymentTransaction m_mPaymentTransaction = copyFrom(this, new Timestamp(System.currentTimeMillis()), X_C_PaymentTransaction.TRXTYPE_Void, getR_PnRef(), get_TrxName());
	...
	public boolean delayCaptureOnlineAuthorizationPaymentTransaction(int C_Invoice_ID)
	{
		if (getTenderType().equals(X_C_PaymentTransaction.TENDERTYPE_CreditCard) && isOnline() && getTrxType().equals(X_C_PaymentTransaction.TRXTYPE_Authorization) && !isVoided() && !isDelayedCapture())
		{
			...
			try
			{
				MPaymentTransaction m_mPaymentTransaction = copyFrom(this, new Timestamp(System.currentTimeMillis()), X_C_PaymentTransaction.TRXTYPE_DelayedCapture, getR_PnRef(), get_TrxName());
				m_mPaymentTransaction.setIsApproved(false);
				m_mPaymentTransaction.setIsVoided(false);
				m_mPaymentTransaction.setIsDelayedCapture(false);
	...
	public String getCreditCardName(String CreditCardType)
	{
		if (CreditCardType == null)
			...
			return "MasterCard";
		else if (X_C_PaymentTransaction.CREDITCARDTYPE_Visa.equals(CreditCardType))
			return "Visa";
		else if (X_C_PaymentTransaction.CREDITCARDTYPE_Amex.equals(CreditCardType))
			return "Amex";
		else if (X_C_PaymentTransaction.CREDITCARDTYPE_ATM.equals(CreditCardType))
			return "ATM";
		else if (X_C_PaymentTransaction.CREDITCARDTYPE_Diners.equals(CreditCardType))
			return "Diners";
	...
	public static int[] getAuthorizationPaymentTransactionIDs(int C_Order_ID, int C_Invoice_ID, String trxName)
	{
		...
		return MPaymentTransaction.getAllIDs(I_C_PaymentTransaction.Table_Name, whereClause.toString(), trxName);
	}
