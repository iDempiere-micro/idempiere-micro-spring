		...
		*/
		...
		if (MDocType.DOCSUBTYPEINV_InternalUseInventory.equals(docSubTypeInv)) {
			...
			if (getQtyInternalUse().signum() != 0) {
				log.saveError("Quantity", Msg.getElement(getCtx(), I_M_InventoryLine.COLUMNNAME_QtyInternalUse));
				return false;
			}
			...
			if (MAcctSchema.COSTINGLEVEL_BatchLot.equals(costingLevel)) {				
				if (M_ASI_ID == 0) {
					log.saveError("FillMandatory", Msg.getElement(getCtx(), I_M_InventoryLine.COLUMNNAME_M_AttributeSetInstance_ID));
					return false;
				}
			}
