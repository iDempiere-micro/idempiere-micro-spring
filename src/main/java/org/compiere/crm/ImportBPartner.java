...
public class ImportBPartner extends SvrProcess implements ImportProcess {
  ...
  private boolean m_createAllLocations = false;

  /** import Id to be the only one imported */
  private int i_bPartner_Id = 0;
  ...
  protected void prepare() {
    ...
    for (int i = 0; i < para.length; i++) {
      ...
      if (name.equals("AD_Client_ID")) {
        Object para1 = para[i].getParameter();
        if (para1 instanceof BigDecimal)
          m_AD_Client_ID = ((BigDecimal) para[i].getParameter()).intValue();
        if (para1 instanceof Integer) m_AD_Client_ID = para[i].getParameterAsInt();
      } else if (name.equals("DeleteOldImported"))
        m_deleteOldImported = "Y".equals(para[i].getParameter());
      else if (name.equals("CreateAllLocations"))
        m_createAllLocations = "Y".equals(para[i].getParameter());
      else if (name.equals("IsValidateOnly")) p_IsValidateOnly = para[i].getParameterAsBoolean();
      else if (name.equals("i_bPartner_Id")) i_bPartner_Id = para[i].getParameterAsInt();
  ...
  protected String doIt() throws java.lang.Exception {
    ...
    String idClause = "";
    if (i_bPartner_Id > 0) {
      idClause = " AND I_BPartner_Id = " + i_bPartner_Id;
      clientCheck += idClause;
    }
    ...
    if (m_deleteOldImported) {
      sql =
          new StringBuilder("DELETE I_BPartner ")
    ...
    sql =
        new StringBuilder("UPDATE I_BPartner ")
    ...
    //	Set BP_Group
    sql =
        new StringBuilder("UPDATE I_BPartner i ")
    ...
    //	Set Country
    /**
     * sql = new StringBuffer ("UPDATE I_BPartner i " + "SET CountryCode=(SELECT CountryCode FROM
     * C_Country c WHERE c.IsDefault='Y'" + " AND c.AD_Client_ID IN (0, i.AD_Client_ID) AND
     * ROWNUM=1) " + "WHERE CountryCode IS NULL AND C_Country_ID IS NULL" + " AND
     * I_IsImported<>'Y'").append(clientCheck); no = DB.executeUpdateEx(sql.toString(),
    ...
    sql =
        new StringBuilder("UPDATE I_BPartner i ")
            ...
            .append("WHERE i.EMail IS NOT NULL AND I_IsImported='N'")
            .append(clientCheck);
    no = DB.executeUpdateEx(sql.toString(), get_TrxName());
    ...
    //	Go through Records
    sql =
        new StringBuilder("SELECT * FROM I_BPartner ")
    ...
    try {
      ...
      while (rs.next()) {
        ...
        StringBuilder msglog =
            new StringBuilder("I_BPartner_ID=")
                .append(impBP.getI_BPartner_ID())
                .append(", C_BPartner_ID=")
                .append(impBP.getC_BPartner_ID())
                .append(", C_BPartner_Location_ID=")
                .append(impBP.getC_BPartner_Location_ID())
                .append(", AD_User_ID=")
                .append(impBP.getAD_User_ID());
        if (log.isLoggable(Level.FINE)) log.fine(msglog.toString());

        if (m_createAllLocations || !New_BPValue.equals(Old_BPValue)) {
          if (!New_BPValue.equals(Old_BPValue)) {
            ...
            if (impBP.getC_BPartner_ID() == 0) // 	Insert new BPartner
            {
              ...
              ModelValidationEngine.get()
                  .fireImportValidate(this, impBP, bp, ImportValidator.TIMING_AFTER_IMPORT);
          ...
          if (impBP.getC_BPartner_Location_ID() != 0) // 	Update Location
          {
            ...
            ModelValidationEngine.get()
                .fireImportValidate(this, impBP, bpl, ImportValidator.TIMING_AFTER_IMPORT);
            bpl.saveEx();
          } else //	New Location
          if (impBP.getC_Country_ID() != 0
            ...
            MLocation location =
                new MLocation(
                    getCtx(),
                    impBP.getC_Country_ID(),
                    impBP.getC_Region_ID(),
                    impBP.getCity(),
                    get_TrxName());
            location.setAddress1(impBP.getAddress1());
            location.setAddress2(impBP.getAddress2());
            ...
            if (bpl.save()) {
              msglog =
                  new StringBuilder("Insert BP Location - ")
                      .append(bpl.getC_BPartner_Location_ID());
              if (log.isLoggable(Level.FINEST)) log.finest(msglog.toString());
        ...
        if (impBP.getAD_User_ID() != 0) {
          ...
          ModelValidationEngine.get()
              .fireImportValidate(this, impBP, user, ImportValidator.TIMING_AFTER_IMPORT);
          if (user.save()) {
