...
public class X_C_OrderTax extends PO implements I_Persistent
{
    ...
    {
      POInfo poi = POInfo.getPOInfo (ctx, I_C_OrderTax.Table_ID, get_TrxName());
      return poi;
    }
	...
	{
		if (C_Order_ID < 1) 
			set_ValueNoCheck (I_C_OrderTax.COLUMNNAME_C_Order_ID, null);
		else 
			set_ValueNoCheck (I_C_OrderTax.COLUMNNAME_C_Order_ID, Integer.valueOf(C_Order_ID));
	}
	...
	{
		if (C_Tax_ID < 1) 
			set_ValueNoCheck (I_C_OrderTax.COLUMNNAME_C_Tax_ID, null);
		else 
			set_ValueNoCheck (I_C_OrderTax.COLUMNNAME_C_Tax_ID, Integer.valueOf(C_Tax_ID));
	}
	...
	public void setC_TaxProvider_ID (int C_TaxProvider_ID)
	{
		if (C_TaxProvider_ID < 1) 
			set_ValueNoCheck (I_C_OrderTax.COLUMNNAME_C_TaxProvider_ID, null);
		else 
