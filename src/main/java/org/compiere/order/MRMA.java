...
public class MRMA extends X_M_RMA implements I_M_RMA {
  ...
  public MRMATax[] getTaxes(boolean requery) {
    ...
    //
    List<MRMATax> list =
        new Query(getCtx(), I_M_RMATax.Table_Name, "M_RMA_ID=?", get_TrxName())
            .setParameters(getId())
            .list();
  ...
  public int copyLinesFrom(MRMA otherRMA, boolean counter) {
    ...
    for (int i = 0; i < fromLines.length; i++) {
      ...
      else copyValues(fromLine, line, fromLine.getADClientID(), fromLine.getAD_Org_ID());
  ...
  public MRMALine[] getChargeLines() {
    ...
    int rmaLineIds[] =
        MRMALine.getAllIDs(MRMALine.Table_Name, whereClause.toString(), get_TrxName());
