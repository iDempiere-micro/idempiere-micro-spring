...
public class ShippingUtil {
  ...
  public static int findRecipientAccountLocationId(
      int shipper_id, int c_bpartner_id, int ad_org_id, int inout_id, int order_id) {
    int accountLocationId =
        DB.getSQLValue(
            ...
            "SELECT C_BPartner_Location_ID "
                ...
                + "   AND AD_Org_ID = "
                + ad_org_id
                + " AND M_ShippingProcessor_ID IN (SELECT DISTINCT M_ShippingProcessor_ID FROM M_Shipper WHERE M_Shipper_ID = "
                + shipper_id
                + ")"
                //			 + "   AND M_Shipper_ID = " + shipper_id
                + "   AND C_BPartner_Location_ID Is Not Null",
            c_bpartner_id);
      ...
      accountLocationId =
          DB.getSQLValue(
              ...
              "SELECT bps.C_BPartner_Location_ID"
                  ...
                  + " AND bps.M_ShippingProcessor_ID IN (SELECT DISTINCT M_ShippingProcessor_ID FROM M_Shipper WHERE M_Shipper_ID = "
                  + shipper_id
                  + ")"
