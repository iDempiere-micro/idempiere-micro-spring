...
public class X_M_RMATax extends PO implements I_Persistent
{
    ...
    {
      POInfo poi = POInfo.getPOInfo (ctx, I_M_RMATax.Table_ID, get_TrxName());
      return poi;
    }
	...
	{
		if (C_Tax_ID < 1) 
			set_ValueNoCheck (I_M_RMATax.COLUMNNAME_C_Tax_ID, null);
		else 
			set_ValueNoCheck (I_M_RMATax.COLUMNNAME_C_Tax_ID, Integer.valueOf(C_Tax_ID));
	}
	...
	public void setC_TaxProvider_ID (int C_TaxProvider_ID)
	{
		if (C_TaxProvider_ID < 1) 
			set_ValueNoCheck (I_M_RMATax.COLUMNNAME_C_TaxProvider_ID, null);
		else 
	...
	{
		if (M_RMA_ID < 1) 
			set_ValueNoCheck (I_M_RMATax.COLUMNNAME_M_RMA_ID, null);
		else 
			set_ValueNoCheck (I_M_RMATax.COLUMNNAME_M_RMA_ID, Integer.valueOf(M_RMA_ID));
	}
