...
public class MLookupFactory {
  ...
  private static CCache<String, MLookupInfo> s_cacheRefTable =
      new CCache<String, MLookupInfo>(I_AD_Ref_Table.Table_Name, 30, 60); // 	1h
  ...
  public static MLookup get(
    ...
    MLookupInfo info =
        getLookupInfo(
            ...
            IsParent,
            ValidationCode);
    if (info == null) throw new Exception("MLookup.create - no LookupInfo");
    return new MLookup(info, 0);
  } //  create

  public static MLookupInfo getLookupInfo(
      Properties ctx, int WindowNo, int Column_ID, int AD_Reference_ID) {
    return getLookupInfo(ctx, WindowNo, 0, Column_ID, AD_Reference_ID);
  }

  public static MLookupInfo getLookupInfo(
      Properties ctx, int WindowNo, int TabNo, int Column_ID, int AD_Reference_ID) {
  ...
  /**
   * ************************************************************************ Get Information for
   * Lookups based on Column_ID for Table Columns or Process Parameters.
   *
   * <p>The SQL returns three columns:
   *
   * <pre>
  ...
  public static MLookupInfo getLookupInfo(
    ...
    if (info.ValidationCode == null) info.ValidationCode = "";
    ...
    if (info.ValidationCode.length() == 0) info.IsValidated = true;
    else {
      /*
    ...
    if (needToAddSecurity)
      info.Query =
          MRole.getDefault(ctx, false)
              .addAccessSQL(info.Query, info.TableName, MRole.SQL_FULLYQUALIFIED, MRole.SQL_RO);
    //
  ...
  public static MLookupInfo getLookup_List(Language language, int AD_Reference_Value_ID) {
    String byValue =
        DB.getSQLValueString(
            null,
            "SELECT IsOrderByValue FROM AD_Reference WHERE AD_Reference_ID = ? ",
            AD_Reference_Value_ID);
    ...
    if (Env.isBaseLanguage(language, "AD_Ref_List"))
      ...
      realSQL
          .append("trl.Name, AD_Ref_List.IsActive ")
    ...
    if ("Y".equals(byValue)) realSQL.append(" ORDER BY 2");
    else realSQL.append(" ORDER BY 3"); // sort by name/translated name - teo_sarca, [ 1672820 ]
    //
    MLookupInfo info =
        new MLookupInfo(
            realSQL.toString(),
            ...
            101,
            101,
            MQuery.getEqualQuery("AD_Reference_ID", AD_Reference_Value_ID)); // 	Zoom Window+Query
   ...
   */
  public static String getLookup_ListEmbed(
      Language language, int AD_Reference_Value_ID, String linkColumnName) {
    ...
    if (Env.isBaseLanguage(language, "AD_Ref_List"))
      ...
      realSQL
          ...
          .append("')");
    realSQL
        .append(" WHERE AD_Ref_List.AD_Reference_ID=")
        .append(AD_Reference_Value_ID)
        .append(" AND AD_Ref_List.Value=")
        .append(linkColumnName);
   ...
   * @param WindowNo window no
   * @param AD_Reference_Value_ID reference value
   * @return SELECT Key, NULL, Name, IsActive FROM Table - if KeyColumn end with _ID otherwise
   *     SELECT NULL, Key, Name, IsActive FROM Table
   */
  private static MLookupInfo getLookup_Table(
      Properties ctx, Language language, int WindowNo, int AD_Reference_Value_ID) {
    String lang;
    if (language == null) {
    ...
    StringBuilder key =
        new StringBuilder()
            .append(Env.getADClientID(ctx))
            .append("|")
            .append(Env.getAD_Role_ID(ctx))
            .append("|")
    ...
    //
    String sql0 =
        "SELECT t.TableName,ck.ColumnName AS KeyColumn," //	1..2
    ...
    StringBuilder realSQL = new StringBuilder("SELECT ");
    if (!KeyColumn.endsWith("_ID")) realSQL.append("NULL,");
    ...
    if (showID) {
      ...
      if (list == null || list.size() == 0) {
        ...
        if (list == null) list = new ArrayList<LookupDisplayColumn>();
    ...
    if (WhereClause != null && WhereClause.length() > 0) {
      ...
      if (where.indexOf('@') != -1) where = Env.parseContext(ctx, WindowNo, where, false);
      ...
      if (where.length() != 0) {
        ...
        if (where.indexOf('.') == -1)
          s_log.log(
              ...
              "getLookup_Table - "
                  + TableName
                  + ": WHERE should be fully qualified: "
                  + WhereClause);
    ...
    if (OrderByClause != null && OrderByClause.length() > 0) {
      ...
      if (OrderByClause.indexOf('.') == -1)
        s_log.log(
            Level.SEVERE,
            "getLookup_Table - " + TableName + ": ORDER BY must fully qualified: " + OrderByClause);
    } else realSQL.append(" ORDER BY 3");
    ...
    retValue =
        new MLookupInfo(
            realSQL.toString(), TableName, msginf.toString(), ZoomWindow, ZoomWindowPO, zoomQuery);
   ...
   */
  public static String getLookup_TableEmbed(
      Language language, String BaseColumn, String BaseTable, int AD_Reference_Value_ID) {
    String sql =
        "SELECT t.TableName,ck.ColumnName AS KeyColumn,"
    ...
    try {
      ...
      if (!rs.next()) {
        s_log.log(
            Level.SEVERE,
            "Cannot find Reference Table, ID="
                + AD_Reference_Value_ID
                + ", Base="
                + BaseTable
                + "."
    ...
    if (isValueDisplayed) embedSQL.append(TableNameAlias).append(".Value||'-'||");
    ...
    if (IsTranslated && !Env.isBaseLanguage(language, TableName)) {
      ...
      if (columnDisplay.isVirtualColumn()) {
        ...
        return null;
      } else embedSQL.append(TableName).append("_Trl.").append(DisplayColumn);
    ...
    //	Not Translated
      ...
      else embedSQL.append(TableNameAlias).append(".").append(DisplayColumn);
   ...
   */
  private static MLookupInfo getLookup_TableDir(
      Properties ctx, Language language, int WindowNo, String ColumnName) {
  ...
  private static StringBuilder getDisplayColumn(
      Language language, String tableName, ArrayList<LookupDisplayColumn> list) {
    return getDisplayColumn(language, tableName, list, tableName);
  }

  private static StringBuilder getDisplayColumn(
      Language language, String TableName, ArrayList<LookupDisplayColumn> list, String baseTable) {
    ...
    for (int i = 0; i < size; i++) {
      if (i > 0) {
        displayColumn
            ...
            .append(
                MSysConfig.getValue(
                    MSysConfig.IDENTIFIER_SEPARATOR, "_", Env.getADClientID(Env.getCtx())))
            .append("'|| ");
      }
      ...
      //  translated
      if (ldc.IsTranslated
          && !Env.isBaseLanguage(language, TableName)
          && !ldc.IsVirtual
          && baseTable != null
          && !(TableName + "_Trl").equalsIgnoreCase(baseTable)) {
      ...
      //	Table
      else if ((ldc.DisplayType == DisplayType.Table || ldc.DisplayType == DisplayType.Search)
          && ldc.AD_Reference_ID != 0) {
        String embeddedSQL;
        if (ldc.IsVirtual)
          embeddedSQL =
              getLookup_TableEmbed(language, ldc.ColumnSQL, TableName, ldc.AD_Reference_ID);
        else
          embeddedSQL =
              getLookup_TableEmbed(language, ldc.ColumnName, TableName, ldc.AD_Reference_ID);
        if (embeddedSQL != null) displayColumn.append("(").append(embeddedSQL).append(")");
      }
      //  TableDir
        ...
        else embeddedSQL = getLookup_TableDirEmbed(language, ldc.ColumnName, TableName);
        if (embeddedSQL != null) displayColumn.append("(").append(embeddedSQL).append(")");
      }
      //	ID
   ...
   */
  public static String getLookup_TableDirEmbed(
      Language language, String ColumnName, String BaseTable) {
   ...
   */
  public static String getLookup_TableDirEmbed(
      Language language, String ColumnName, String BaseTable, String BaseColumn) {
    String KeyColumn = MQuery.getZoomColumnName(ColumnName);
    String TableName = MQuery.getZoomTableName(ColumnName);
        ...
        && !(TableName + "_Trl").equalsIgnoreCase(BaseTable)) // IDEMPIERE-1070
    {
      embedSQL
          ...
          .append(TableName)
          .append("_Trl.AD_Language=")
          .append(DB.TO_STRING(language.getADLanguage()))
          .append(")");
    }
    embedSQL.append(" WHERE ");
    // If is not virtual column - teo_sarca [ 1739530 ]
  ...
  private static ArrayList<LookupDisplayColumn> getListIdentifiers(String TableName) {
    ...
    for (String idColumnName : table.getIdentifierColumns()) {
      ...
      LookupDisplayColumn ldc =
          new LookupDisplayColumn(
  ...
  public static String getInfoString(Properties ctx, String tableName, int id, String trxName) {
    ...
    String sql =
        MLookupFactory.getLookup_TableDirEmbed(language, tableName + "_ID", "[?", "?]")
            .replace("[?.?]", "?");
