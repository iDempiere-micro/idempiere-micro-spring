...
public class MWorkflow extends X_AD_Workflow
{
	...
	)
	{
		...
		if (s_cacheDocValue.isReset())
		{
			...
			List<MWorkflow> workflows = new Query(ctx, I_AD_Workflow.Table_Name, whereClause, trxName)
	...
	/**	Single Cache					*/
	private static CCache<String,MWorkflow>	s_cache = new CCache<String,MWorkflow>(I_AD_Workflow.Table_Name, 20);
	/**	Document Value Cache			*/
	private static CCache<String,MWorkflow[]>	s_cacheDocValue = new CCache<String,MWorkflow[]> (I_AD_Workflow.Table_Name, I_AD_Workflow.Table_Name+"_Document_Value", 5);
	/**	Static Logger	*/
	...
	public long getDurationBaseSec ()
	{
		if (getDurationUnit() == null)
			return 0;
		else if (X_AD_Workflow.DURATIONUNIT_Second.equals(getDurationUnit()))
			return 1;
		else if (X_AD_Workflow.DURATIONUNIT_Minute.equals(getDurationUnit()))
			return 60;
		else if (X_AD_Workflow.DURATIONUNIT_Hour.equals(getDurationUnit()))
			return 3600;
		else if (X_AD_Workflow.DURATIONUNIT_Day.equals(getDurationUnit()))
			return 86400;
		else if (X_AD_Workflow.DURATIONUNIT_Month.equals(getDurationUnit()))
			return 2592000;
		else if (X_AD_Workflow.DURATIONUNIT_Year.equals(getDurationUnit()))
			return 31536000;
		return 0;
	}	//	getDurationBaseSec
