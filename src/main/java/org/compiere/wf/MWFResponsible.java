...
public class MWFResponsible extends X_AD_WF_Responsible
{
	...
	/**	Cache						*/
	private static CCache<Integer,MWFResponsible>	s_cache	= new CCache<Integer,MWFResponsible>(I_AD_WF_Responsible.Table_Name, 10);
	...
	//		return true;
		...
		//	User not used
		if (!X_AD_WF_Responsible.RESPONSIBLETYPE_Human.equals(getResponsibleType()) && getAD_User_ID() > 0)
			setAD_User_ID(0);
		
		//	Role not used
		if (!X_AD_WF_Responsible.RESPONSIBLETYPE_Role.equals(getResponsibleType()) && getAD_Role_ID() > 0)
			setAD_Role_ID(0);
