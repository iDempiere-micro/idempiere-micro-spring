...
/**
 * Add UUID column (tableName_UU) to table and update existing records with new UUID. Warning: this
 * process is only safe to run if it have exclusive access to database.
 *
 * @author hengsin
 */
public class UUIDGenerator extends SvrProcess {
  ...
  protected void prepare() {
    for (IProcessInfoParameter param : getParameter()) {
      ...
      else log.log(Level.SEVERE, "Unknown Parameter: " + param.getParameterName());
    }
  }
  ...
  protected String doIt() throws Exception {
    if (tableName == null || tableName.trim().length() == 0) tableName = "%";
    else tableName = tableName.trim();
    String sql =
        "SELECT AD_Table_ID, TableName FROM AD_Table WHERE TableName LIKE ? AND IsView = 'N' AND IsActive='Y' ORDER BY TableName";
    ...
    try {
      ...
      while (rs.next()) {
        ...
        if (AD_Column_ID <= 0) {
          ...
          int AD_Element_ID =
              DB.getSQLValue(
                  null, "SELECT AD_Element_ID FROM AD_Element WHERE ColumnName=?", columnName);
          if (AD_Element_ID <= 0) {
    ...
    try {
      ...
      try {
        ...
        while (rs.next()) {
          ...
          sql =
              new StringBuilder()
                  .append(column.getSQLModify(table, column.isMandatory() != notNull));
          break;
        }
      } finally {
        DB.close(rs);
      ...
      //	No Table
      if (noColumns == 0) sql = new StringBuilder(table.getSQLCreate());
      //	No existing column
      else if (sql == null) sql = new StringBuilder(column.getSQLAdd(table));
