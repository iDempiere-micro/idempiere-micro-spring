...
public class MProduct extends X_M_Product implements I_M_Product
{
	...
	{
		List<MProduct> list = new Query(ctx, I_M_Product.Table_Name, whereClause, trxName)
								.setClient_ID()
								.list();
	...
	public static MProduct forS_Resource_ID(Properties ctx, int S_Resource_ID, String trxName)
	{
		...
		// Load from DB
		MProduct p = new Query(ctx, I_M_Product.Table_Name, I_M_Product.COLUMNNAME_S_Resource_ID+"=?", trxName)
	...
	/**	Cache						*/
	private static CCache<Integer,MProduct> s_cache	= new CCache<Integer,MProduct>(I_M_Product.Table_Name, 40, 5);	//	5 minutes
