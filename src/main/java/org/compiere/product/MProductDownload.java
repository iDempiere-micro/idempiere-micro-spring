...
public class MProductDownload extends X_M_ProductDownload {
  ...
  public static void migrateDownloads(Properties ctx) {
    ...
    int count = 0;
    sql =
        "SELECT AD_Client_ID, AD_Org_ID, M_Product_ID, Name, DownloadURL "
            + "FROM M_Product "
    ...
    try {
      ...
      while (rs.next()) {
        ...
        if (pdl.save()) {
          count++;
          String sqlUpdate =
              "UPDATE M_Product SET DownloadURL = NULL WHERE M_Product_ID=" + M_Product_ID;
  ...
  public URI getDownloadURL(String directory) {
    ...
    URI url = null;
    try {
      if (dl_url.indexOf("://") != -1) url = new URI(dl_url);
      else {
