...
public class ProductNotOnPriceListException extends AdempiereException
{
	...
	private static final String buildMessage (IProductPricing pp, int documentLineNo)
	{
		...
		if (pp.getM_Product_ID() > 0)
		{
			...
			if (sb.length() > 0)
				sb.append(", ");
			sb.append("@M_Product_ID@:").append(p == null ? "?" : p.get_Translation(HasName.Companion.getCOLUMNNAME_Name()));
		}
		if (pp.getM_PriceList_ID() > 0)
		{
			...
			if (sb.length() > 0)
				sb.append(", ");
			sb.append("@M_PriceList_ID@:").append(pl == null ? "?" : pl.get_Translation(HasName.Companion.getCOLUMNNAME_Name()));
		}
