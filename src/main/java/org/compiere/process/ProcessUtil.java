...
public final class ProcessUtil {
  ...
  public static boolean startDatabaseProcedure(
      IProcessInfo processInfo, String ProcedureName, Trx trx) {
   ...
   */
  public static boolean startDatabaseProcedure(
      IProcessInfo processInfo, String ProcedureName, Trx trx, boolean managedTrx) {
    ...
    try {
      ...
      processInfo.setSummary(
          Msg.getMsg(Env.getCtx(), "ProcessRunError") + " " + e.getLocalizedMessage());
      processInfo.setError(true);
      return false;
    } finally {
      DB.close(cstmt);
      cstmt = null;
      if (trx != null && managedTrx) trx.close();
    }
    return true;
  }
  ...
  public static boolean startJavaProcess(
      Properties ctx, IProcessInfo pi, Trx trx, boolean managedTrx) {
    return startJavaProcess(ctx, pi, trx, managedTrx, null);
  }

  public static boolean startJavaProcess(
      Properties ctx, IProcessInfo pi, Trx trx, boolean managedTrx, IProcessUI processMonitor) {
  ...
  public static boolean startScriptProcess(Properties ctx, IProcessInfo pi, Trx trx) {
    ...
    try {
      ...
      engine.put(MRule.ARGUMENTS_PREFIX + "AD_Client_ID", pi.getADClientID());
