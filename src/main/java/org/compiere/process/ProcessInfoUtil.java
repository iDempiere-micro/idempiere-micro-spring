...
public class ProcessInfoUtil {
  ...
  public static void saveLogToDB(IProcessInfo pi) {
    ...
    for (int i = 0; i < logs.length; i++) {
      ...
      else sql.append(DB.TO_DATE(logs[i].getP_Date(), false)).append(",");
      ...
      else sql.append(DB.TO_STRING(logs[i].getP_Msg(), 2000)).append(",");
  ...
  public static void setParameterFromDB(IProcessInfo pi) {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        //
        if (pi.getADClientID() == null) pi.setADClientID(rs.getInt(10));
