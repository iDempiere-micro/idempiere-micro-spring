...
public abstract class SvrProcess implements ProcessCall {
  ...
  protected void commitEx() throws SQLException {
    if (m_trx != null) m_trx.commit(true);
  }
  ...
  protected int getADClientID() {
    if (m_pi.getADClientID() == null) {
      ...
      if (m_pi.getADClientID() == null) return 0;
    }
  ...
  private void unlock() {
    ...
    try {
      ...
      if (noContext) Env.getCtx().put("#AD_Client_ID", m_pi.getADClientID());
  ...
  public String get_TrxName() {
    if (m_trx != null) return m_trx.getTrxName();
    return null;
  } //	get_TrxName
