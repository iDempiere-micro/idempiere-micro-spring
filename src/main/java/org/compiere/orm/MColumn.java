...
public class MColumn extends X_AD_Column
{
	...
	/**	Cache						*/
	private static CCache<Integer,MColumn>	s_cache	= new CCache<Integer,MColumn>(I_AD_Column.Table_Name, 20);
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if (!isVirtualColumn() && getValueMax() != null && getValueMin() != null)
		{
			try {
				...
				{
					log.saveError("MaxLessThanMin", Msg.getElement(getCtx(), I_AD_Column.COLUMNNAME_ValueMax));
					return false;
				}
			} catch (Exception e){}
		}
		...
		**/
		...
		if (isIdentifier())
		{
			...
			if (cnt>0)
			{
				log.saveError(DBException.SAVE_ERROR_NOT_UNIQUE_MSG, Msg.getElement(getCtx(), I_AD_Column.COLUMNNAME_SeqNo));
				return false;
			}
		}
							...
							if (primaryKey != null)
							{
								...
								if (fkConstraintType == null) {
									...
									if (fkConstraintType == null) {
										if (   "AD_Client_ID".equals(column.getColumnName())
											...
											fkConstraintType = X_AD_Column.FKCONSTRAINTTYPE_NoAction;
									}
								}
								if (!fkConstraintType.equals(X_AD_Column.FKCONSTRAINTTYPE_DoNotCreate))
								{
									...
									if (   dbForeignKey.getKeyName().equalsIgnoreCase(column.getFKConstraintName())
										&& (   (dbForeignKey.getDeleteRule() == DatabaseMetaData.importedKeyCascade && X_AD_Column.FKCONSTRAINTTYPE_Cascade.equals(column.getFKConstraintType()))
										    || (dbForeignKey.getDeleteRule() == DatabaseMetaData.importedKeySetNull && X_AD_Column.FKCONSTRAINTTYPE_SetNull.equals(column.getFKConstraintType()))
										    || (dbForeignKey.getDeleteRule() == DatabaseMetaData.importedKeyNoAction && X_AD_Column.FKCONSTRAINTTYPE_NoAction.equals(column.getFKConstraintType()))
										    || (dbForeignKey.getDeleteRule() == DatabaseMetaData.importedKeyRestrict && X_AD_Column.FKCONSTRAINTTYPE_NoAction.equals(column.getFKConstraintType()))
										   )
									   ) {
										// nothing changed
										return "";
