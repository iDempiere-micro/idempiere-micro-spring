...
public class MDocType extends X_C_DocType
{
	...
	{
		List<MDocType> list = new Query(ctx, I_C_DocType.Table_Name, null, null)
									.setClient_ID()
									.setOnlyActiveRecords(true)
									.list();
		return list.toArray(new MDocType[list.size()]);
	...
	/**	Cache					*/
	static private CCache<Integer,MDocType>	s_cache = new CCache<Integer,MDocType>(I_C_DocType.Table_Name, 20);
	...
	{
		boolean isSOTrx = X_C_DocType.DOCBASETYPE_SalesOrder.equals(getDocBaseType())
	...
	public String getNameTrl()
	{
		...
		return get_Translation (I_C_DocType.HasName.Companion.getCOLUMNNAME_Name(), Env.getAD_Language(getCtx()));
	}	//	getNameTrl
