...
public class AccessSqlParser {
  ...
  private TableInfo[] getTableInfo(String sql) {
    ...
    while (fromIndex != -1) {
      ...
      while (tableST.hasMoreTokens()) {
        ...
        else tableInfo = new TableInfo(tableString);
  ...
  public String toString() {
    ...
    if (m_tableInfo == null) sb.append(m_sqlOriginal);
    else {
  ...
  public int getMainSqlIndex() {
    if (m_sql == null) return -1;
    else if (m_sql.length == 1) return 0;
    for (int i = m_sql.length - 1; i >= 0; i--) {
      if (m_sql[i].charAt(0) != '(') return i;
    }
    return -1;
  } //	getMainSqlIndex
  ...
  public String getMainSql() {
    ...
    if (m_sql.length == 1) return m_sql[0];
    for (int i = m_sql.length - 1; i >= 0; i--) {
      if (m_sql[i].charAt(0) != '(') return m_sql[i];
    }
    return "";
  } //	getMainSql
  ...
  public static class TableInfo {
    ...
    public String toString() {
      ...
      if (getSynonym().length() > 0) sb.append("=").append(m_synonym);
      return sb.toString();
    } //	toString
  } //	TableInfo
} //	AccessSqlParser
