...
public class DefaultModelFactory implements IModelFactory {
  ...
  private static CCache<String, Class<?>> s_classCache =
      new CCache<String, Class<?>>(null, "PO_Class", 20, false);
  private static final CLogger s_log = CLogger.getCLogger(DefaultModelFactory.class);

  /** Packages for Model Classes */
  private static final String[] s_packages =
      new String[] {
        "org.compiere.model",
        "org.compiere.impexp",
  ...
  public Class<?> getClass(String tableName, Boolean useCache) {
    ...
    if (useCache) {
      ...
      if (cache != null) {
        ...
        if (cache.equals(Object.class)) return null;
        else return cache;
      }
    }
    ...
    if (tableName.startsWith("I_")) {
      ...
      if (etmodelpackage == null || MEntityType.ENTITYTYPE_Dictionary.equals(entityType))
        etmodelpackage =
            "org.compiere.impl"; // fallback for dictionary or empty model package on entity type
