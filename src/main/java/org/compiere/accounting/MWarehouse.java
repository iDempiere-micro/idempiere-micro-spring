...
public class MWarehouse extends X_M_Warehouse
{
	...
	public static MWarehouse[] getForOrg (Properties ctx, int AD_Org_ID)
	{
		...
		List<MWarehouse> list = new Query(ctx, I_M_Warehouse.Table_Name, whereClause, null)
	...
	/**	Cache					*/
	private static CCache<Integer,MWarehouse> s_cache = new CCache<Integer,MWarehouse>(I_M_Warehouse.Table_Name, 50 );
	/**
	...
	public I_M_Locator[] getLocators(boolean reload)
	{
		...
		List<PO> list = new Query(getCtx(), I_M_Locator.Table_Name, whereClause, null)
