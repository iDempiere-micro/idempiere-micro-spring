...
public class MAcctSchemaElement extends X_C_AcctSchema_Element
{
	...
	/**	Cache						*/
	private static CCache<Integer,MAcctSchemaElement[]> s_cache = new CCache<Integer,MAcctSchemaElement[]>(I_C_AcctSchema_Element.Table_Name, 10);
	...
	{
		String et = getElementType();
		if (X_C_AcctSchema_Element.ELEMENTTYPE_UserColumn1.equals(et) || X_C_AcctSchema_Element.ELEMENTTYPE_UserColumn2.equals(et))
		{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		String et = getElementType();
		if (isMandatory() &&
			(X_C_AcctSchema_Element.ELEMENTTYPE_UserElementList1.equals(et) || X_C_AcctSchema_Element.ELEMENTTYPE_UserElementList2.equals(et)
			|| X_C_AcctSchema_Element.ELEMENTTYPE_UserColumn1.equals(et) || X_C_AcctSchema_Element.ELEMENTTYPE_UserColumn2.equals(et)))
			setIsMandatory(false);
		// Acct Schema Elements "Account" and "Org" should be mandatory - teo_sarca BF [ 1795817 ]
		if (X_C_AcctSchema_Element.ELEMENTTYPE_Account.equals(et) || X_C_AcctSchema_Element.ELEMENTTYPE_Organization.equals(et)) {
			if (!isMandatory())
				setIsMandatory(true);
			if (!isActive())
				setIsActive(true);
		...
		else if (isMandatory())
		{
			...
			else if (X_C_AcctSchema_Element.ELEMENTTYPE_LocationFrom.equals(et) && getC_Location_ID() == 0)
			...
			else if (X_C_AcctSchema_Element.ELEMENTTYPE_SalesRegion.equals(et) && getC_SalesRegion_ID() == 0)
	...
	private void updateData (String element, int id)
	{
		...
		StringBuilder sql = new StringBuilder("UPDATE C_ValidCombination SET ").append(element).append("=").append(id)
			.append(" WHERE ").append(element).append(" IS NULL AND AD_Client_ID=").append(getADClientID());
