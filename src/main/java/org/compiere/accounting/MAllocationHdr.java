		...
		if (!isReversal()) {
			...
			{	
				if (line.getC_Invoice_ID() != 0)
				{
					...
					.setClient_ID()
					.setParameters(line.getC_Invoice_ID(), "Y", I_C_Invoice.DOCSTATUS_Voided, I_C_Invoice.DOCSTATUS_Reversed)
					.match();
		...
		for (MAllocationLine line : m_lines) {
			...
			if (C_Payment_ID > 0)
			{
				...
				if (paymentProcessed)
				{
					if (invoice != null)
					{
						...
						if (amt == null)
						{
							m_processMsg = MConversionRateUtil.getErrorMessage(getCtx(), "ErrorConvertingAllocationCurrencyToBaseCurrency",
									getC_Currency_ID(), org.compiere.accounting.MClient.get(getCtx()).getC_Currency_ID(), convTypeID, paymentDate, get_TrxName());
							return false;
						}
					...
					{
						m_processMsg = MConversionRateUtil.getErrorMessage(getCtx(), "ErrorConvertingAllocationCurrencyToBaseCurrency",
								getC_Currency_ID(), org.compiere.accounting.MClient.get(getCtx()).getC_Currency_ID(), convTypeID, getDateAcct(), get_TrxName());
						return false;
					}
			...
			else if (invoice != null)
			{
				...
				{
					m_processMsg = MConversionRateUtil.getErrorMessage(getCtx(), "ErrorConvertingAllocationCurrencyToBaseCurrency",
							getC_Currency_ID(), org.compiere.accounting.MClient.get(getCtx()).getC_Currency_ID(), invoice.getC_ConversionType_ID(), invoice.getDateAcct(), get_TrxName());
					return false;
				}
				openBalanceDiff = openBalanceDiff.add(amt);
			}
					 ...
					 (getC_Currency_ID() != invoice.getC_Currency_ID())))
			{
				...
				if (invAmtAccted == null)
				{
					m_processMsg = MConversionRateUtil.getErrorMessage(getCtx(), "ErrorConvertingInvoiceCurrencyToBaseCurrency",
							invoice.getC_Currency_ID(), org.compiere.accounting.MClient.get(getCtx()).getC_Currency_ID(), invoice.getC_ConversionType_ID(), invoice.getDateAcct(), get_TrxName());
					return false;
				}
