...
public class MAcctSchema extends X_C_AcctSchema
{
	...
	private static CCache<Integer,MAcctSchema> s_cache = new CCache<Integer,MAcctSchema>(I_C_AcctSchema.Table_Name, 3, 120, true);	//  3 accounting schemas
...
//		m_onlyOrgs = orgs;
	...
	public boolean isCostingLevelClient()
	{
		...
		if (s == null || X_C_AcctSchema.COSTINGLEVEL_Client.equals(s))
			return true;
		return false;
	...
	public String getTaxCorrectionType()
	{
		if (super.getTaxCorrectionType() == null)	//	created 07/23/06 2.5.3d
			setTaxCorrectionType(isDiscountCorrectsTax() 
				? X_C_AcctSchema.TAXCORRECTIONTYPE_Write_OffAndDiscount : X_C_AcctSchema.TAXCORRECTIONTYPE_None);
