	...
	implements DocAction, ProcessCall, PaymentInterface, IPODoc
{
	...
	public boolean processOnline()
	{
		...
		if(getTrxType().equals(X_C_Payment.TRXTYPE_Void) || getTrxType().equals(X_C_Payment.TRXTYPE_CreditPayment))
		{
			if (isVoided())
			{
		...
		try
		{
			IPaymentProcessor pp = MPaymentTransaction.createPaymentProcessor(m_mBankAccountProcessor, this);
			if (pp == null)
...
*/
		...
		try
		{
			...
			for (int i = 0; i < m_mBankAccountProcessors.length; i++)
			{
				...
				if (bankAccountProcessor.isAcceptAMEX() && paymentProcessor.isAcceptAMEX())
					map.put (X_C_Payment.CREDITCARDTYPE_Amex, getCreditCardPair (X_C_Payment.CREDITCARDTYPE_Amex));
				if (bankAccountProcessor.isAcceptDiners() && paymentProcessor.isAcceptDiners())
					map.put (X_C_Payment.CREDITCARDTYPE_Diners, getCreditCardPair (X_C_Payment.CREDITCARDTYPE_Diners));
				if (bankAccountProcessor.isAcceptDiscover() && paymentProcessor.isAcceptDiscover())
					map.put (X_C_Payment.CREDITCARDTYPE_Discover, getCreditCardPair (X_C_Payment.CREDITCARDTYPE_Discover));
				if (bankAccountProcessor.isAcceptMC() && paymentProcessor.isAcceptMC())
					map.put (X_C_Payment.CREDITCARDTYPE_MasterCard, getCreditCardPair (X_C_Payment.CREDITCARDTYPE_MasterCard));
				if (bankAccountProcessor.isAcceptCorporate() && paymentProcessor.isAcceptCorporate())
					map.put (X_C_Payment.CREDITCARDTYPE_PurchaseCard, getCreditCardPair (X_C_Payment.CREDITCARDTYPE_PurchaseCard));
				if (bankAccountProcessor.isAcceptVisa() && paymentProcessor.isAcceptVisa())
					map.put (X_C_Payment.CREDITCARDTYPE_Visa, getCreditCardPair (X_C_Payment.CREDITCARDTYPE_Visa));
	...
	public String getCreditCardName(String CreditCardType)
	{
		if (CreditCardType == null)
			return "--";
		else if (X_C_Payment.CREDITCARDTYPE_MasterCard.equals(CreditCardType))
			return "MasterCard";
		else if (X_C_Payment.CREDITCARDTYPE_Visa.equals(CreditCardType))
			return "Visa";
		else if (X_C_Payment.CREDITCARDTYPE_Amex.equals(CreditCardType))
			return "Amex";
		else if (X_C_Payment.CREDITCARDTYPE_ATM.equals(CreditCardType))
			return "ATM";
		else if (X_C_Payment.CREDITCARDTYPE_Diners.equals(CreditCardType))
			return "Diners";
		else if (X_C_Payment.CREDITCARDTYPE_Discover.equals(CreditCardType))
			return "Discover";
		else if (X_C_Payment.CREDITCARDTYPE_PurchaseCard.equals(CreditCardType))
			return "PurchaseCard";
