...
public class MMeasure extends X_PA_Measure
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		else if (X_PA_Measure.MEASURETYPE_Ratio.equals(getMeasureType())
			&& getPA_Ratio_ID() == 0)
		{
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		...
		if (success && X_PA_Measure.MEASURETYPE_Manual.equals(getMeasureType()))
			updateManualGoals();
	...
	public boolean updateGoals()
	{
		...
		try
		{
			if (X_PA_Measure.MEASURETYPE_Manual.equals(mt))
				return updateManualGoals();
			else if (X_PA_Measure.MEASURETYPE_Achievements.equals(mt))
				return updateAchievementGoals();
			else if (X_PA_Measure.MEASURETYPE_Calculated.equals(mt))
				return updateCalculatedGoals();
			else if (X_PA_Measure.MEASURETYPE_Ratio.equals(mt))
				return updateRatios();
			else if (X_PA_Measure.MEASURETYPE_Request.equals(mt))
				return updateRequests();
			else if (X_PA_Measure.MEASURETYPE_Project.equals(mt))
				return updateProjects();
			else if(X_PA_Measure.MEASURETYPE_UserDefined.equals(mt))
	...
	{
		if (!X_PA_Measure.MEASURETYPE_Manual.equals(getMeasureType()))
			return false;
	...
	{
		if (!X_PA_Measure.MEASURETYPE_Achievements.equals(getMeasureType()))
			return false;
	...
	{
		if (!X_PA_Measure.MEASURETYPE_Ratio.equals(getMeasureType()))
			return false;
		return false;
	}		//	updateRatios
	...
	{
		if (!X_PA_Measure.MEASURETYPE_Request.equals(getMeasureType())
			|| getR_RequestType_ID() == 0)
			return false;
		MGoal[] goals = MGoal.getMeasureGoals (getCtx(), getPA_Measure_ID());
		for (int i = 0; i < goals.length; i++)
	...
	{
		if (!X_PA_Measure.MEASURETYPE_Project.equals(getMeasureType())
			|| getC_ProjectType_ID() == 0)
			return false;
		MGoal[] goals = MGoal.getMeasureGoals (getCtx(), getPA_Measure_ID());
		for (int i = 0; i < goals.length; i++)
