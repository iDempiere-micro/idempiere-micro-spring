...
public class MPPProductBOM extends X_PP_Product_BOM
{
	...
	/**	Cache						*/
	private static CCache<Integer,MPPProductBOM> s_cache = new CCache<Integer,MPPProductBOM>(I_PP_Product_BOM.Table_Name, 40, 5);
	/** BOM Lines					*/
	...
	public static List<MPPProductBOM> getProductBOMs(MProduct product)
	{
		...
		return new Query (product.getCtx(), I_PP_Product_BOM.Table_Name, whereClause, product.get_TrxName())
					.setClient_ID()
	...
	{
		MPPProductBOM bom = new Query(product.getCtx(), I_PP_Product_BOM.Table_Name, "M_Product_ID=? AND Value=?", trxName)
	...
	{
		int count = new Query(getCtx(), I_PP_Product_BOM.Table_Name, I_PP_Product_BOM.COLUMNNAME_M_Product_ID+"=?", get_TrxName())
