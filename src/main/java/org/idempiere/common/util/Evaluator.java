...
public class Evaluator {
  ...
  public static boolean evaluateLogic(Evaluatee source, String logic) {
    ...
    while (st.hasMoreTokens()) {
      ...
      if (logOp.equals("&")) retValue = retValue & temp;
      else if (logOp.equals("|")) retValue = retValue | temp;
      else {
  ...
  private static boolean evaluateLogicTuple(Evaluatee source, String logic) {
    ...
    if (st.countTokens() != 3) {
      s_log.log(
          Level.SEVERE,
          "Logic tuple does not comply with format "
              + "'@context@=value' where operand could be one of '=!^><' => "
              + logic);
      return false;
    }
    //	First Part
    ...
    for (String second : list) {
      ...
      if (first.trim().endsWith("_ID") && firstEval.length() == 0) firstEval = "0";
      if (second.trim().endsWith("_ID") && secondEval.length() == 0) secondEval = "0";
  ...
  private static boolean evaluateLogicTuple(String value1, String operand, String value2) {
    ...
    try {
      if (!value1.startsWith("'")) value1bd = new BigDecimal(value1);
      if (!value2.startsWith("'")) value2bd = new BigDecimal(value2);
    } catch (Exception e) {
      value1bd = null;
      value2bd = null;
    }
    //
    if (operand.equals("=")) {
      if (value1bd != null && value2bd != null) return value1bd.compareTo(value2bd) == 0;
