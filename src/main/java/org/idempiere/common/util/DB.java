 ...
 *     <li>Modifications: removed static references to database connection and instead always get a
...
public final class DB {
  ...
  public static String DB_VERSION = "2017-10-31";
  /** Product Name */
   ...
   *     <p>// Role update log.info("Role"); String sql = "SELECT * FROM AD_Role"; PreparedStatement
   ...
   *     <p>// Language check log.info("Language"); MLanguage.maintain(ctx);
   *     <p>// Sequence check log.info("Sequence"); ProcessInfo processInfo = new
   ...
   *     processInfo, null);
   *     <p>// Costing Setup log.info("Costing"); MAcctSchema[] ass =
   *     MAcctSchema.getClientAcctSchema(ctx, 0); for (int i = 0; i < ass.length; i++) {
   *     ass[i].checkCosting(); ass[i].saveEx(); }
   *     <p>// Reset Flag system.setIsJustMigrated(false); return system.save(); } // afterMigration
   */
  ...
  public static synchronized void setDBTarget(CConnection cc) {
    if (cc == null) throw new IllegalArgumentException("Connection is NULL");
  ...
  public static boolean isConnected(boolean createNew) {
    ...
    if (eb != null && eb.isIssueError()) eb.setIssueError(false);
    else eb = null; // 	don't reset
    try {
  ...
  public static String getDatabaseInfo() {
    if (s_cc != null) return s_cc.getDbInfo();
    return "No Database";
  } //	getDatabaseInfo
  ...
  public static boolean isDatabaseOK(Properties ctx) {
    ...
    if (DB_VERSION.equals(version)) return true;
  ...
  public static boolean isBuildOK(Properties ctx) {
    ...
    //  Identical DB version
    if (buildClient.equals(buildDatabase)) return true;

    String AD_Message = "BuildVersionError";
    String title =
        AD_Message; // DAP TODO org.idempiere.Adempiere.getName() + " " +  Msg.getMsg(ctx,
    // AD_Message, true);
    // The program assumes build version {0}, but database has build Version {1}.
    String msg = AD_Message; // TODO DAP Msg.getMsg(ctx, AD_Message);   //  complete message
  ...
  public static CallableStatement prepareCall(
      String SQL, int resultSetConcurrency, String trxName) {
   ...
   * @deprecated
   */
  public static CPreparedStatement prepareStatement(
   ...
   */
  public static CPreparedStatement prepareStatement(
      String sql, int resultSetType, int resultSetConcurrency, String trxName) {
    if (sql == null || sql.length() == 0) throw new IllegalArgumentException("No SQL");
    //
  ...
  /**
   * Set PreparedStatement's parameter. Similar with calling <code>pstmt.setObject(index, param)
   * </code>
   *
   * @param pstmt
   * @param index
  ...
  public static void setParameter(PreparedStatement pstmt, int index, Object param)
      throws SQLException {
    if (param == null) pstmt.setObject(index, null);
    else if (param instanceof String) pstmt.setString(index, (String) param);
    else if (param instanceof Integer) pstmt.setInt(index, ((Integer) param).intValue());
    else if (param instanceof BigDecimal) pstmt.setBigDecimal(index, (BigDecimal) param);
    else if (param instanceof Timestamp) pstmt.setTimestamp(index, (Timestamp) param);
    else if (param instanceof Boolean)
      pstmt.setString(index, ((Boolean) param).booleanValue() ? "Y" : "N");
    else if (param instanceof byte[]) pstmt.setBytes(index, (byte[]) param);
    else throw new DBException("Unknown parameter type " + index + " - " + param);
  }
  ...
  public static int executeUpdate(
      String sql, int param, boolean ignoreError, String trxName, int timeOut) {
   ...
   */
  public static int executeUpdate(
      String sql, Object[] params, boolean ignoreError, String trxName) {
   ...
   */
  public static int executeUpdate(
      String sql, Object[] params, boolean ignoreError, String trxName, int timeOut) {
    if (sql == null || sql.length() == 0)
      throw new IllegalArgumentException("Required parameter missing - " + sql);
   ...
   * @throws SQLException
   */
  public static int executeUpdateEx(String sql, Object[] params, String trxName)
      throws DBException {
   ...
   * @throws SQLException
   */
  public static int executeUpdateEx(String sql, Object[] params, String trxName, int timeOut)
      throws DBException {
    if (sql == null || sql.length() == 0)
      throw new IllegalArgumentException("Required parameter missing - " + sql);
    //
   ...
   * @throws SQLException
   */
  public static boolean commit(boolean throwException, String trxName)
  ...
  public static boolean rollback(boolean throwException, String trxName) throws SQLException {
    try {
      ...
      if (trx != null) return trx.rollback(true);
      else conn = DB.getConnectionRW();
  ...
  public static RowSet getRowSet(String sql) {
    ...
    CStatementVO info =
        new CStatementVO(
            ...
            DB.getDatabase().convertStatement(sql));
   ...
   * @throws DBException if there is any SQLException
   */
  public static BigDecimal getSQLValueBDEx(String trxName, String sql, Object... params)
      throws DBException {
   ...
   * @throws DBException if there is any SQLException
   */
  public static BigDecimal getSQLValueBDEx(String trxName, String sql, List<Object> params)
      throws DBException {
  ...
  public static Timestamp getSQLValueTSEx(String trxName, String sql, Object... params) {
    ...
    try {
      ...
      setParameters(pstmt, params);
      rs = pstmt.executeQuery();
      if (rs.next()) retValue = rs.getTimestamp(1);
      else if (log.isLoggable(Level.FINE)) log.fine("No Value " + sql);
    } catch (SQLException e) {
      throw new DBException(e, sql);
    } finally {
   ...
   * @throws DBException if there is any SQLException
   */
  public static Timestamp getSQLValueTSEx(String trxName, String sql, List<Object> params)
      throws DBException {
  ...
  public static KeyNamePair[] getKeyNamePairs(
      String trxName, String sql, boolean optional, Object... params) {
    PreparedStatement pstmt = null;
    ResultSet rs = null;
   ...
   *
   * <p>Deprecated, always return false.
   *
  ...
  public static Exception getSQLException(Exception e) {
    Throwable e1 = e;
    while (e1 != null) {
      if (e1 instanceof SQLException) return (SQLException) e1;
      e1 = e1.getCause();
    }
    return e;
  }
  ...
  public static ValueNamePair[] getValueNamePairs(
      String sql, boolean optional, List<Object> params) {
    PreparedStatement pstmt = null;
    ResultSet rs = null;
   ...
   * @param trxName
   */
  public static void createT_Selection(
      int AD_PInstance_ID, Collection<Integer> selection, String trxName) {
   ...
   * @param trxName
   */
  public static void createT_SelectionNew(
      int AD_PInstance_ID, Collection<KeyNamePair> saveKeys, String trxName) {
    StringBuilder insert = new StringBuilder();
  ...
  public static boolean isTableOrViewExists(String tableName) {
    ...
    try {
      ...
      String tblName;
      if (metadata.storesUpperCaseIdentifiers()) tblName = tableName.toUpperCase();
      else if (metadata.storesLowerCaseIdentifiers()) tblName = tableName.toLowerCase();
      else tblName = tableName;
    ...
    try {
      ...
      if (rs.next()) {
        for (int i = 1; i <= rsmd.getColumnCount(); i++) {
          ...
          if (rs.wasNull()) retValue.add(null);
          else retValue.add(obj);
        }
      } else {
        retValue = null;
      }
   ...
   * @throws DBException if there is any SQLException
   */
  public static List<List<Object>> getSQLArrayObjectsEx(
      String trxName, String sql, Object... params) {
    ...
    if (rowsArray.size() == 0) return null;
    return rowsArray;
  }
} //	DB
