...
public class EnvLoader {
   ...
   *
   * <p>Prints error messages on System.err
   *
  ...
  public static Properties getEnv(String prefix) {
    ...
    if (!System.getProperty("os.name", "").startsWith("Win")) cmd = "set"; //  Unix/Linux
  ...
  private static String execCommand(String command) {
    ...
    if (DEBUG) System.out.println("** Command executed: " + command);
    ...
    try {
      ...
      //
      int c;
      while ((c = in.read()) != -1) bufOut.append((char) c);
      in.close();
      //
      while ((c = err.read()) != -1) bufErr.append((char) c);
      err.close();
    } catch (Exception e) {
  ...
  private static Properties parseEnv(String input, String prefix) {
    ...
    while (st.hasMoreTokens()) {
      ...
      if (pos > 0) prop.setProperty(prefix + s.substring(0, pos), s.substring(pos + 1));
    }
    if (DEBUG) System.out.println("** Loaded " + prop.size() + " Properties");
    return prop;
  } //  parseEnv
} //  EnvLoader
