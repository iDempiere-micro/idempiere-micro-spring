...
public class AmtInWords_HR implements AmtInWords {
  ...
  /**
   * Vraća dani broj napisan slovima, u valuti (kune). Broj mora biti između -10^100 i 10^100. Bit
   * će automatski zaokružen na dvije decimale.
   *
   * @param broj
  ...
  /**
   * Vraća dani broj napisan slovima. Broj mora biti između -10^100 i 10^100. Decimale se ignoriraju
   * (dakle režu, a ne zaokružuju).
   *
   * @param broj
   * @return broj napisan slovima
   */
   ...
   *
   * @param broj
   * @param jedinica u nominativu ("kuna", "lipa", "tisuća", "milijun"); može biti null
   * @return broj napisan slovima
   */
  ...
  /** Privatne i cacheirane varijable - pune se samo kod prvog instanciranja klase. */
  private static final BigDecimal googol = valueOf(10).pow(100);

  private static final BigDecimal dvadeset = valueOf(20);
  ...
  private static final String[][] nominativi = {
    // redom:
  ...
  */
  /**
   * Vraća dani broj napisan slovima. Broj mora biti između -10^66 i 10^66. Decimale se ignoriraju
   * (dakle režu, a ne zaokružuju).
   *
   * @param broj
   * @param jedinica u nominativu ("kuna", "lipa", "tisuća", "milijun"); može biti null
   * @param jediniceBezBroja false kada je jedinica valuta, a ne brojčana jedinica
   * @param rekurzija true kad ova metoda poziva samu sebe
   * @return broj napisan slovima
   */
  private static StringBuilder slovima(
      BigDecimal broj, String jedinica, boolean jediniceBezBroja, boolean rekurzija) {
    StringBuilder rezultat = new StringBuilder();
    ...
    if (googol.compareTo(broj) < 1) {
      ...
      throw new IllegalArgumentException(
          "nepoznat broj - nije između-googol i googol (-10^100 i 10^100)");
    }
    ...
    if (broj.compareTo(valueOf(1)) == 0) {
      ...
      // slučaju)
      if (!jediniceBezBroja && rekurzija) rezultat.append(I.length() > 1 ? I.substring(1) : I);
      ...
      rezultat.append(
          slovima(
              broj.subtract(ostatak).movePointLeft(punaPotencija),
  ...
  private static int potencija(BigDecimal broj) {
    ...
    for (BigDecimal i = deset; i.compareTo(broj) < 1; i = i.multiply(deset)) potencija++;

    return potencija;
  }

  /**
   * Deklinira imenice koje završavaju na -a ili -n u nominativu. Ako je nominativ null, onda vraća
   * samo broj u muškom rodu.
   *
   * @param znamenka 0-20
   * @param nominativ ("kuna", "lipa", "tisuća", "milijun"); može biti null
   * @param jedinceBezBroja ako je ovaj flag dignut, onda se preskače broj i vraća se samo jedinica,
   *     npr. "milijarda" umjesto "jedna milijarda"
    ...
    if (nominativ == null)
      ...
      for (int i = 0; i < nominativi.length; i++) {
        if (nominativ.endsWith(nominativi[i][0])) {
          ...
          if (nominativ.endsWith("a")) broj = brojevi.get(znamenka * -1);

          // ako nema ženske varijante broja, onda je broj isti kao i
          // kod muškog roda
          if (broj == null) broj = brojevi.get(znamenka);
