...
public class StatementProxy implements InvocationHandler {
	...
	private SimpleDateFormat format = (SimpleDateFormat)DateFormat.getDateTimeInstance(DateFormat.SHORT, DateFormat.SHORT);
	...
	public Object invoke(Object obj, Method method, Object[] args)
		...
		if (log.isLoggable(Level.FINE) && getSql() != null)
		{
			if (name.equals("executeUpdate") || name.equals("execute"))
			{
				...
				if (logOperation != null) {
					...
					if (log.isLoggable(Level.FINE)) log.fine(format.format(new Date(System.currentTimeMillis()))+","+logOperation+","+logSql+","+(p_vo.getTrxName() != null ? p_vo.getTrxName() : "")+" (begin)");
				}
			}
		}
		...
		finally
		{
			...
			{
				log.fine(format.format(new Date(System.currentTimeMillis()))+","+logOperation+","+logSql+","+(p_vo.getTrxName() != null ? p_vo.getTrxName() : "")+" (end)");
			}
		}
	}
