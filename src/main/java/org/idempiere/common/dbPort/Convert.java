 ...
 */
public abstract class Convert implements IConvert {
    ...
    try {
      ...
      for (i = 0; ok && i < sql.length; i++) {
        ...
        if (statement.length() == 0) {
          if (m_verbose) if (log.isLoggable(Level.FINER)) log.finer("Skipping empty (" + i + ")");
        } else {
          if (m_verbose) {
          ...
          try {
            ...
            if (m_verbose) if (log.isLoggable(Level.FINE)) log.fine("- ok " + no);
  ...
  public String convertAll(String sqlStatements) {
    ...
    for (int i = 0; i < sql.length; i++) {
      ...
      if (m_verbose) if (log.isLoggable(Level.INFO)) log.info("Statement " + i + ": " + sql[i]);
    }
    return sb.toString();
  } //  convertAll
  ...
  public static synchronized void logMigrationScript(String oraStatement, String pgStatement) {
    ...
    if (logMigrationScript) {
      ...
      try {
        ...
        // writeLogMigrationScript(writerOr, oraStatement); DAP TODO
      } catch (IOException e) {
        e.printStackTrace();
      }
      try {
        if (pgStatement == null) {
          ...
          IConvert convert = Database.getDatabase(Database.DB_POSTGRESQL).getConvert();
        ...
        // writeLogMigrationScript(writerPg, pgStatement); DAP TODO
      } catch (IOException e) {
        e.printStackTrace();
      }
    }
